...
<x-checkout::editable-box
    ...
    onEdit="reopen('{{Section::Traveller->value}}')"
>
        ...
        @foreach($paxInfo as $roomNumber => $room)
                    ...
                    <div class="mb-3">
                        <div role="tablist" aria-label="Travellers"
                            ...
                            @foreach($room as $i => ['showTraveller' => $trav])
                                ...
                                >
                                    <svg
                                        class="@if($trav->isShowing) w-5 h-5 text-gray-700 @else w-5 h-5 text-gray-400 group-hover:text-gray-600 @endif"
          ...
          {{ $trav->isAdult ? trans('checkout::page.trip_details.adult') : trans('checkout::page.trip_details.child') }}
                                    ...
                                    @if($itinerary->hasRentalCar() && $i === 0 )
                                        <span
                                            class="ml-2 px-2.5 py-0.5 text-xs font-semibold text-gray-800 bg-gray-200 rounded-md border border-gray-300">
                ...
                @foreach($room as $i => ['showTraveller' => $showTraveller])
                    <div @if(! $showTraveller->isShowing) class="hidden" @endif>
                        ...
                        @if(config()->boolean('checkout::cuba-travel.active'))
                            @include('checkout::components.cuba-travel.reason',  ['model' => "paxInfo.$roomNumber.$i"])
                        @endif
                        ...
                        <div class="flex justify-between items-center mt-8">
                            ...
                            <button type="button"
                                    wire:click="showNextTraveller('{{$roomNumber}}-{{$i}}')"
                                    wire:target="showNextTraveller('{{$roomNumber}}-{{$i}}')"
                                    wire:loading.attr="disabled"
                                    wire:loading.class="opacity-70 cursor-not-allowed"
                                    class="inline-flex items-center justify-center gap-2 bg-blue-500 hover:bg-blue-600 text-white px-6 py-2 rounded-md">
                                <svg wire:loading wire:target="showNextTraveller('{{$roomNumber}}-{{$i}}')" class="h-4 w-4 animate-spin text-white" viewBox="0 0 24 24" fill="none" aria-hidden="true">
