...
class GetItineraryResponse extends BaseDto
{
    ...
    public function countAdults(): int
    {
        return $this->paxDetails
            ->reject(fn (PaxDetailEntity $pax): bool => $pax->age !== null && $pax->age < 18)
            ->count();
    }
    ...
    {
        return $this->paxDetails
            ->filter(fn (PaxDetailEntity $pax): bool => $pax->age !== null && $pax->age < 18)
            ->count();
    }
    ...
    {
        return $this->paxDetails
            ->filter(fn (PaxDetailEntity $detail): bool => $detail->age !== null && $detail->age < 18)
            ->pluck('age')
            ->values();
    }
}
